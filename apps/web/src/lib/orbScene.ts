...
export function createOrbScene(container: HTMLElement, options?: OrbSceneOptions): OrbSceneApi {
  ...
  function scatterText(count: number, sizeFn: () => number, rFn: () => number, speedScale: [number, number]) {
    ...
    for (let i = 0; i < count; i++) {
      const snippet = codeSnippets[Math.floor(Math.random() * codeSnippets.length)] ?? "0x00000000";
  ...
  function animate() {
    ...
    try {
      ...
      if (chromaticPass.uniforms && chromaticPass.uniforms["uTime"]) {
